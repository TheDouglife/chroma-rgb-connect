...
public class ChromaConnectNamedPipeClientTests
{
    [Fact]
    public async Task ConnectNamedPipeAsyncReturnsConnectedStreamWhenServerIsAvailable()
    {
        var pipeName = $"ChromaConnect.Tests.{Guid.NewGuid():N}";
        ...
        await using var clientStream = await ChromaConnectExtensions.ConnectNamedPipeAsync(pipeName, clientCancellationSource.Token);
    ...
    public async Task ConnectNamedPipeAsyncPreservesCancellation()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => ChromaConnectExtensions.ConnectNamedPipeAsync(pipeName, cancellationTokenSource.Token));
    }
    ...
    public async Task ConnectNamedPipeAsyncWrapsPipeConnectionFailuresWithContext()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => ChromaConnectExtensions.ConnectNamedPipeAsync(
            pipeName,
